...
// ---------------------------------------------------------------------------
// TODO(multi-page export): once a single chart gets too dense/wide to read
// on one sheet, split it into one page per calendar year instead of one
// giant page. Plan, so this doesn't need re-deriving later:
//
// 1. Chunk `parsed` by calendar year (parsed[i].date.getFullYear()) into
//    an array of arrays — one sub-array per year that has entries.
//
// 2. Compute the y-axis range ONCE, over the FULL dataset (not per-year),
//    so every page shares the same scale and pages stay visually
//    comparable at a glance:
//      const allWeights = parsed.map(d => d.weight);
//      const globalMinW = Math.min(...allWeights), globalMaxW = Math.max(...allWeights);
//
// 3. Give buildChartSVG(...) an optional 4th/5th param — yMinOverride /
//    yMaxOverride — and have it skip its own `minW`/`maxW` derivation when
//    those are passed in, using the shared values instead. (Everything
//    else — margins, xStepDays, per-page date range — stays per-chunk,
//    since each page's x-axis should only span its own year.)
//
// 4. Call buildChartSVG(yearChunk, xStepDaysOverride, yStepLbs, globalMinW, globalMaxW)
//    once per year-chunk, producing one SVG per page.
//
// 5. In buildExportDocument(), instead of one `${svg}`, loop the chunks and
//    wrap each page's SVG (plus a small "2023" year label) in its own
//    `<div class="export-page">...</div>`. Add to the print CSS:
//      .export-page { page-break-after: always; }
//      .export-page:last-child { page-break-after: auto; }
//    The existing `@page { size: 17in 11in; margin: 0.5in; }` rule applies
//    per page automatically — no change needed there.
//
// 6. The MAX/CURRENT/MIN stats row stays exactly as-is: computed once over
//    the full `parsed` array, shown once at the top, independent of how
//    the chart itself gets paginated below it.
// ---------------------------------------------------------------------------
